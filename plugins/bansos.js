...
let handler = async (m, { conn, args, usedPrefix, DevMode }) => {
  try {
    ...
    if (new Date - global.db.data.users[m.sender].lastbansos > 300000) {
      if (Aku > Kamu) {
        conn.sendFile( m.chat, kbansos, 'b.jpg', `Kamu Tertangkap Setelah Kamu korupsi dana bansos, Dan kamu harus membayar denda 3 Juta rupiah`, m)
        user.money -= 3000000
        ...
        conn.sendMessage(m.chat, `Maaf kamu tidak mendapatkan *Bansos* dan kamu tidak masuk penjara karna melarikan diri\n${botol}`, m)
        global.db.data.users[m.sender].lastbansos = new Date * 1
      }
    } else m.reply(`Kamu sudah melakukan *Korupsi Bansos*\nDan kamu harus menunggu selama agar bisa korupsi bansos kembali ${timers}`)
  } catch (e) {
    throw `${e}`
  }
}
