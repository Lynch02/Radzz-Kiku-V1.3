...
• *Balance Leaderboard Top ${len}* •
Kamu: *${usersBalance.indexOf(m.sender) + 1}* dari *${usersBalance.length}*

${sortedBalance.slice(0, len).map(({ jid, balance }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *Balance ${balance}*`).join`\n`}
`.trim()
